...
//$runningRoutine = startLogRoutine('activateUpcomingArticles');
$runningRoutine = startLogRoutine('analyticArticles');
...
while(!feof($fileOpen))
{
    ...
    /*$line = str_replace('portalId', 'portal_id', $line);
    $line = str_replace('pageId', 'page_id', $line);
    $line = str_replace('pageType', 'page_type', $line);
    $line = str_replace('sessionId', 'session_id', $line);*/
    ...
    {
        $sql = "INSERT INTO analytic (portal_id, page_type, page_id, page, ip, session_id, date, fulldate,
        referrer, browser)
                VALUES ('".$data['portalId']."', '".$data['pageType']."', '".$data['pageId']."', '".$data['page']."',
                        '".$data['ip']."', '".$data['sessionId']."', '".$data['date']."', '".$data['fulldate']."',
                        '".$data['referrer']."', '".$data['browser']."')";
        echo $sql;
